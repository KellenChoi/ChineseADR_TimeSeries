...
####################### Getting Data ###################################
getSymbols("PPCCF", from="2013-01-01", to="2018-02-28")
class(PPCCF)
str(PPCCF)
head(PPCCF)
tail(PPCCF)
PPCCF$PPCCF.Close[1:5]
# dates -> index
dt=index(PPCCF); length(dt); dt[1]; dt[length(dt)]
sum(is.na(PPCCF)) ## NO MISSING 
######################### Plotting ######################################
#plot data
chartSeries(PPCCF, type = c("auto", "candlesticks", "matchsticks", "bars","line"), 
            subset = NULL,
            show.grid = TRUE, 
            name = NULL,
            log.scale = FALSE,
...
chartSeries(PPCCF, subset='2014-01-01/2014-12-31', type='line', name ='PPCCF.Close', TA=NULL)
chartSeries(PPCCF, subset='last 4 months', type='bars', name ='PPCCF.Close', TA=NULL)
chartSeries(PPCCF, subset='last 4 months', theme="black", TA=NULL) #draw the chart 
addSMA()
addVo() #add volume 
...
PPCCF.c=PPCCF$PPCCF.Close
plot(PPCCF.c, main='PPCCF.Close')

# add the 50 period simple moving average to panel 1 of the plot
lines(SMA(PPCCF.c, n = 50), on=1, col="green")

# add month end points to the chart
points(PPCCF.c[endpoints(PPCCF.c, on = "months")], col="red", pch=40, on=1)
...
# simple return
simple.ret = Delt(PPCCF.c, type="arithmetic")[-1] #simple return R = P(t)/P(t-1) - 1
# plot the close and add a panel with the simple returns
plot(PPCCF.c, main='PPCCF.Close')
lines(simple.ret, type="h", on=NA)

# log return
log.ret = Delt(PPCCF.c, type="log")[-1]  #log return r = ln(1+R) = ln(P(t)/P(t-1))
# plot the close and add a panel with the simple returns
plot(PPCCF.c, main='PPCCF.Close')
...
df.tmp = data.frame(PPCCF$PPCCF.Volume[-1])
df.tmp$PPCCF.lrt <- log.ret
PPCCF.xts.lrt = as.xts(df.tmp$PPCCF.lrt)
chartSeries(PPCCF.xts.lrt, subset='last 4 years', theme="white", TA=NULL, type='line', name ='PPCCF.log.ret')
...
#############################################################################
PPCCF.c.v=as.numeric(PPCCF.c);
acf(PPCCF.c.v)
plot(PPCCF.c.v, type='l')
summary(PPCCF.c.v)
...
mean(PPCCF.c); var(PPCCF.c); skewness(PPCCF.c); kurtosis(PPCCF.c)
...
adf.test(PPCCF.c);#- p=0.1741 # non-stationary
kpss.test(PPCCF.c)  # p << 0.01 #Clearly non-stationary!!!!!!!!!!!!!          

log.ret = Delt(PPCCF.c, type="log")[-1]  
...
############################# Modelling Patterns ##############################
# Plot daily closing prices for Alibab (PPCCF & AMZN)
getSymbols("PPCCF", src="google")
plot(Cl(PPCCF))
...
PPCCFrt = diff(log(Cl(PPCCF)))[-1]
head(PPCCFrt); tail(PPCCFrt)
plot(PPCCFrt)
plot(density(PPCCFrt))
acf(PPCCFrt, na.action=na.omit)
wilcox.test(as.numeric(PPCCFrt))
adf.test(PPCCFrt) #-stationary
kpss.test(PPCCFrt)
Box.test(PPCCFrt) 
...
fit_PPCCF <- fitdist(as.numeric(PPCCFrt), "norm")
gofstat(fit_PPCCF)
...
plot(fit_PPCCF, histo = FALSE, demp = TRUE)
kurtosis(as.numeric(PPCCFrt))  #### not normal!!!!
...
fit_PPCCF <- fitdist(as.numeric(PPCCFrt),"t.scaled",
                     start=list(df=3,mean=mean(as.numeric(PPCCFrt)),sd=sd(as.numeric(PPCCFrt))))
plot(fit_PPCCF, histo = FALSE, demp = TRUE)
fit_PPCCF$estimate 
...
PPCCFrt.ma <- arima(PPCCFrt, order=c(0, 0, 1))
acf(PPCCFrt.ma$res[-1])

# Fit an ARIMA(0, 0, 2) model (i.e. MA(2) ) 
# and plot the correlogram of the residuals
PPCCFrt.ma <- arima(PPCCFrt, order=c(0, 0, 2))
acf(PPCCFrt.ma$res[-1]) 
...
PPCCFfinal.aic <- Inf
PPCCFfinal.order <- c(0,0,0)
for (i in 0:4) for (j in 0:4) {
  PPCCFcurrent.aic <- AIC(arima(PPCCFrt, order=c(i, 0, j)))
  if (PPCCFcurrent.aic < PPCCFfinal.aic) {
    PPCCFfinal.aic <- PPCCFcurrent.aic
    PPCCFfinal.order <- c(i, 0, j)
    PPCCFfinal.arma <- arima(PPCCFrt, order=PPCCFfinal.order)
  }
}

# Output the results of the fit
PPCCFfinal.order  #[1] 2 0 4 

# Plot the residuals of the final model
acf(resid(PPCCFfinal.arma), na.action=na.omit)

# Carry out a Ljung-Box test for realisation
# of discrete white noise
Box.test(resid(PPCCFfinal.arma), lag=20, type="Ljung-Box")

plot(density(PPCCFfinal.arma$residuals))

wilcox.test(resid(PPCCFfinal.arma))
adf.test(resid(PPCCFfinal.arma)) #-stationary
kpss.test(resid(PPCCFfinal.arma))

# Plot the correlogram
acf(abs(resid(PPCCFfinal.arma)))   ## still correlated

#fit to normal
fit_PPCCF <- fitdist(as.numeric(resid(PPCCFfinal.arma)), "norm")
gofstat(fit_PPCCF)
#summary(fit)
plot(fit_PPCCF, histo = FALSE, demp = TRUE)
...
#fit to Student's t distribution for 'df' degrees of freedom, shifted by 'mean' and scaled by 'sd'.
fit_PPCCF <- fitdist(as.numeric(resid(PPCCFfinal.arma)),"t.scaled",
                     start=list(df=3,mean=mean(as.numeric(resid(PPCCFfinal.arma))),sd=sd(as.numeric(resid(PPCCFfinal.arma)))))
plot(fit_PPCCF, histo = FALSE, demp = TRUE)
fit_PPCCF$estimate 
...
for (p in 1:4) for (d in 0:1) for (q in 1:4) {
  PPCCFcurrent.aic <- AIC(arima(log.ret, order=c(p, d, q)))
  if (PPCCFcurrent.aic < PPCCFfinal.aic) {
    PPCCFfinal.aic <- PPCCFcurrent.aic
    PPCCFfinal.order <- c(p, d, q)
    PPCCFfinal.arima <- arima(log.ret, order=PPCCFfinal.order)
  }
}

PPCCFfinal.order  ## 2 0 4 

# Plot a correlogram of the residuals, calculate 
# the Ljung-Box test and predict the next 25 daily
# values of the series
acf(resid(PPCCFfinal.arima), na.action=na.omit)
Box.test(resid(PPCCFfinal.arima), lag=20, type="Ljung-Box") #no correlations!
plot(resid(PPCCFfinal.arima))
acf(abs(resid(PPCCFfinal.arima)))  #still have some unused information
#try to fit to normal distribution
fit_PPCCF <- fitdist(as.numeric(resid(PPCCFfinal.arima)), "norm")
plot(fit_PPCCF, histo = FALSE, demp = TRUE)  # not normal!!!

#fit to Student's t distribution for 'df' degrees of freedom, shifted by 'mean' and scaled by 'sd'.
fit_PPCCF <- fitdist(as.numeric(resid(PPCCFfinal.arima)),"t.scaled",
                     start=list(df=3,mean=mean(as.numeric(resid(PPCCFfinal.arima))),sd=sd(as.numeric(resid(PPCCFfinal.arima)))))
plot(fit_PPCCF, histo = FALSE, demp = TRUE)  # clearly fat-tailedl!!!
##############################################################

#use the forecast library to automatically determine ARIMA coefficients
fit_PPCCF <- auto.arima(log.ret)
fit_PPCCF ## 0 0 0 ???
fit_PPCCF$arima$coef  #forecast pkg yields AR(1)
plot(fit_PPCCF$residuals)
Acf(fit_PPCCF$residuals)  # bad fitting
plot(forecast(PPCCF.c, h=50)) #bad forcast

############################### GARCH ######################################################
# Fit a GARCH model to resid(tencnetfinal.arima)
ft.garch <- garch(resid(PPCCFfinal.arima), trace=F)
ft.res <- ft.garch$res[-1]
length(resid(PPCCFfinal.arima))
#fit to Student's t distribution for 'df' degrees of freedom, shifted by 'mean' and scaled by 'sd'.
...
getSymbols("2328.HK", from="2013-01-01", to="2018-02-28")

head(`2328.HK`)
tail(`2328.HK`)
sum(is.nan(`2328.HK`))  ### no missing values
# dates -> index
dt=index(`2328.HK`); length(dt); dt[1]; dt[length(dt)]

######################### Plotting ######################################
#plot data
chartSeries(`2328.HK`, type = c("auto", "candlesticks", "matchsticks", "bars","line"), 
            subset = NULL,
            show.grid = TRUE, 
            name = NULL,
            log.scale = FALSE,
...
# plot a subset of the data
chartSeries(`2328.HK`, subset='2014-01-01/2014-12-31', type='line', name ='2328.HK.Close', TA=NULL)
chartSeries(`2328.HK`, subset='last 4 months', type='bars', name ='2328.HK.Close', TA=NULL)
chartSeries(`2328.HK`, subset='last 4 months', theme="white", TA=NULL) #draw the chart 
addSMA()
addVo() #add volume 
addBBands() #add Bollinger Bands 
addCCI() #add Commodity Channel Index
